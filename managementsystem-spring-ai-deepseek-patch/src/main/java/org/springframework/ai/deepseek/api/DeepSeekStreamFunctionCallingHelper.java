 ...
 * @author Sun Yuhan
 *
 * DWTech 临时补丁：基于 Spring AI 1.1.5 保留流式工具调用中的
 * reasoning_content，待上游支持后删除本模块。
 */
public class DeepSeekStreamFunctionCallingHelper {
	...
	/**
	 * 将工具调用前已经流出的正文和思考内容补回最终的 tool_calls 消息。
	 */
	public ChatCompletionChunk withAccumulatedMessageContent(ChatCompletionChunk chunk, String content,
			String reasoningContent) {
		if (!isStreamingToolFunctionCall(chunk) || CollectionUtils.isEmpty(chunk.choices())) {
			return chunk;
		}
		...
		ChatCompletionMessage message = new ChatCompletionMessage(
				StringUtils.hasText(content) ? content : delta.content(), delta.role(), delta.name(),
				delta.toolCallId(), delta.toolCalls(), delta.prefix(),
				StringUtils.hasText(reasoningContent) ? reasoningContent : delta.reasoningContent());
		return new ChatCompletionChunk(chunk.id(), List.of(new ChunkChoice(choice.finishReason(), choice.index(),
				message, choice.logprobs())), chunk.created(), chunk.model(), chunk.serviceTier(),
				chunk.systemFingerprint(), chunk.object(), chunk.usage());
	}
	...
	private ChatCompletionMessage merge(ChatCompletionMessage previous, ChatCompletionMessage current) {
		...
		Boolean prefix = (current.prefix() != null ? current.prefix() : previous.prefix());
	...
	private String mergeText(String previous, String current) {
		if (current != null) {
			return previous != null ? previous + current : current;
		}
		return previous;
	}
