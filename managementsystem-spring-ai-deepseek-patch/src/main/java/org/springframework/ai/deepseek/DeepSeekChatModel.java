 ...
 * @author Geng Rong
 *
 * DWTech 临时补丁：基于 Spring AI 1.1.5 保留 DeepSeek V4 thinking
 * tool-call 二次请求所需的 reasoning_content，待上游支持后删除本模块。
 */
public class DeepSeekChatModel implements ChatModel {
	...
	ChatCompletionRequest createRequest(Prompt prompt, boolean stream) {
		...
		boolean propagateReasoningContent = shouldPropagateReasoningContent(requestOptions);
		List<ChatCompletionMessage> chatCompletionMessages = prompt.getInstructions().stream().map(message -> {
			if (message.getMessageType() == MessageType.USER || message.getMessageType() == MessageType.SYSTEM) {
				...
				if (propagateReasoningContent && message instanceof DeepSeekAssistantMessage deepSeekAssistantMessage) {
	...
	private boolean shouldPropagateReasoningContent(DeepSeekChatOptions requestOptions) {
		if (requestOptions == null || !DEEPSEEK_V4_PRO.equals(requestOptions.getModel())) {
			return false;
		}
		return requestOptions.getThinking() == null
				|| !THINKING_DISABLED.equalsIgnoreCase(requestOptions.getThinking().getType());
	}
