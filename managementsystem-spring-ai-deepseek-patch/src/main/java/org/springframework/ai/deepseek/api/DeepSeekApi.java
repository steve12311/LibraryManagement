 ...
 * @author Geng Rong
 *
 * DWTech 临时补丁：基于 Spring AI 1.1.5 扩展 DeepSeek V4 thinking 请求字段，
 * 待上游支持后删除本模块。
 */
public class DeepSeekApi {
	...
	public Flux<ChatCompletionChunk> chatCompletionStream(ChatCompletionRequest chatRequest,
		...
		StringBuilder accumulatedReasoningContent = new StringBuilder();
		...
		return this.webClient.post()
			...
			.onStatus(HttpStatusCode::isError, response -> response.createException()
				.doOnNext(ex -> logger.error("DeepSeek API stream request failed, status={}, body={}",
			...
			.map(chunk -> {
				appendAccumulatedMessageContent(accumulatedContent, accumulatedReasoningContent, chunk);
			...
			.flatMap(mono -> mono)
			.map(chunk -> this.chunkMerger.withAccumulatedMessageContent(chunk, accumulatedContent.toString(),
					accumulatedReasoningContent.toString()));
	}

	private void appendAccumulatedMessageContent(StringBuilder accumulatedContent,
			StringBuilder accumulatedReasoningContent, ChatCompletionChunk chunk) {
		if (chunk == null || chunk.choices() == null || chunk.choices().isEmpty()) {
			return;
		}
		ChatCompletionMessage delta = chunk.choices().get(0).delta();
		if (delta == null) {
			return;
		}
		...
		if (delta.reasoningContent() != null) {
			accumulatedReasoningContent.append(delta.reasoningContent());
		}
	}
	 ...
	 * @param thinking Controls whether DeepSeek V4 thinking mode is enabled.
	 * @param reasoningEffort Controls DeepSeek V4 thinking effort.
	 */
	@JsonInclude(Include.NON_NULL)
	public record ChatCompletionRequest(// @formatter:off
		...
		public ChatCompletionRequest(List<ChatCompletionMessage> messages, Boolean stream) {
			this(messages, null, null, null, null, null,
					null, stream, null, null, null, null, null, null, null, null);
		}
